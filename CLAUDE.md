...
`RENCANA-OPTIMASI-AI.md` (root) adalah tracker optimasi token/biaya AI (status per-item ✅/🔄/⬜): **semua item actionable selesai** — prompt caching trio + extended TTL Claude, plafon `max_tokens` per-aksi, routing model per-tugas, bersih payload Story Bible, cache riwayat chat Claude (#4), debounce/dedup auto-summarizer & rewrite (#6), serta cap lore cache tunable (#5) + override model ringan (#7) lewat Settings → "Optimasi AI Lanjutan". Cek file itu sebelum menyentuh jalur AI terkait token/caching/model.
