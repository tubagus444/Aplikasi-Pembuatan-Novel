...
| ✅ Validasi restore | **SELESAI** — Menambahkan pengecekan versi minimal (>= v3) dan validasi struktur array pada tabel inti (`projects`, `chapters`, `codex`) di dalam `backupService.restoreData` dan `validateProjectBackup`. Eksekusi restore akan menolak backup dengan versi tidak valid atau struktur tidak valid secara aman tanpa mengubah data yang ada. | debt | R | M | `src/services/backupService.ts`, `src/lib/importRemap.ts` |
...
| ✅ tokenWorker langgar aturan C11 | **SELESAI** — `onerror` kini `console.error` + reset status; worker refactor jadi modul singleton di `getSharedWorker()` agar tak *spawn* berulang tiap *mount* | risiko | S | S | `useTokenCounter.ts` |
...
| ✅ Analitik "adegan per wilayah" | **SELESAI** — Modul layer-murni `atlasAnalytics.ts` dibuat untuk menyilangkan `PresenceIndex` dengan area faksi via `pointInPolygon`. Area faksi otomatis mengakumulasikan daftar bab dari penanda area itu sendiri DITAMBAH dengan daftar bab dari penanda `pin` (karakter/item) yang koordinatnya berada di dalam poligon area tersebut. Sesuai prinsip analitik tanpa tabel/FK baru | fitur | S | M | `atlasAnalytics.ts`, `mapGeometry.ts`, `.claude/rules/atlas.md` |
...
| ✅ Pencarian federated | **SELESAI** — Menggabungkan hasil `SemanticSearchPanel` (scene naskah via Cosine Similarity) & Orama BM25 (entri codex via `oramaStore.searchWithScores`) secara paralel. Kedua skor dinormalkan (Min-Max Orama ke skala 0-1) dan disatukan ke dalam `FederatedSearchHit`, lalu ditampilkan bergantian di UI dengan layout card khusus Codex dan Card khusus Scene tanpa perlu overhead query/server baru | fitur | S | M | `SemanticSearchPanel.tsx`, `oramaStore.ts` |
