...
Untuk Peta Kontinuitas, Lensa Karakter, Janji Plot — sekali scan Aho-Corasick atas nama+alias Codex menghasilkan `PresenceIndex` (per-bab `Map<entityId,count>` + per-entitas indeks bab & total). **Jangan scan ganda**; turunkan analitik baru dari sini. Berjalan di main thread on-demand (pola `OrphanScanPanel`), bukan worker.
