      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ref={searchInputRef}
              type="text"
              placeholder={isSemanticMode ? "Cari makna / konteks..." : "Cari..."}
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onKeyDown={handleKeyDown}
              className={cn(
                "w-full h-9 px-3 py-1 bg-slate-50 dark:bg-slate-800/50 border rounded-lg text-sm transition-all focus:outline-none focus:ring-2",
                isSemanticMode ? "focus:ring-fuchsia-500/20 border-fuchsia-200 dark:border-fuchsia-900/50 focus:border-fuchsia-500" : "focus:ring-indigo-500/20",
                !hasResults && !noQuery && !isSemanticMode ? "border-red-300 dark:border-red-900 focus:border-red-500" : (!isSemanticMode ? "border-slate-200 dark:border-slate-700 focus:border-indigo-500" : "")
              )}
            />
            {searchQuery && (
              <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider bg-slate-100 dark:bg-slate-800 px-1.5 rounded">
                {isSemanticMode ? `${searchStats.total} MATCH` : `${searchStats.current} / ${searchStats.total}`}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1">
            <button
              onClick={onPrev}
              disabled={!hasResults || isSemanticMode}
